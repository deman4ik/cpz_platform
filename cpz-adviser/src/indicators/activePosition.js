const activePosition = {
  init() {
    ...
    this.highestSeries = new Array();
    this.highestSeries_size = this.options.highestSeriesSize;
    this.lowestSeries = new Array();
    ...
    this.age = 0;
    this.activeHighSeries = new Array();
    this.activeLowSeries = new Array();  
  ...
  calc() {
    ...
    ////////// debug
    if (this.debug) {
      if (this.entryAction != null) this.log("Debug: ACTIVE " + this.entryAction + "_" + this.activeHighSeries.length);
    }
  },
  setActive (what) {
  ...
  highestHigh (i) {
    ...
    if (this.entryAction != null && this.activeHighSeries.length > 0) {
        if (i > 0) this.log("Error: Indicator ActivePosition.highestHigh: Wrong index, 0 - current, < 0 - previous");
  ...
  lowestLow (i) {
    ...
    if (this.entryAction != null && this.activeLowSeries.length > 0) {
        if (i > 0) this.log("Error: Indicator ActivePosition.lowestLow: Wrong index, 0 - current, < 0 - previous");
