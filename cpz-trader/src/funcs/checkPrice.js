...
class CheckPrice extends BaseService {
  constructor() {
    super();
    this.init();
  }
  ...
  async run(context, { state, data }) {
    try {
      ...
      Log.debug("checkPrice", state, data);
      const trader = new Trader(state);
      trader.checkPrice(data);
      Log.debug("checkPriceResult", trader.currentState);
      Log.clearContext();
