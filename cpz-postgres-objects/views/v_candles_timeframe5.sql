CREATE OR REPLACE VIEW v_candles_timeframe5 AS
  SELECT
    tt.exchange,
    tt.currency,
    tt.asset,
   ...
   FROM ( SELECT
            ...
            t.rownum,
            ((t.rownum + 3) % (5)::bigint) AS test,
            (t.start - (((t.rownum + 3) % (5)::bigint) * 60)) AS grp,
          ...
          WINDOW w AS (
            PARTITION BY ( t.exchange, t.currency, t.asset, t.start - (((t.rownum + 3) % (5)::bigint) * 60) )
