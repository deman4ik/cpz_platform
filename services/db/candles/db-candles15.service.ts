...
const CandlesService: ServiceSchema = {
  name: `${cpz.Service.DB_CANDLES}15`,
  mixins: [DbService],
  adapter: SqlAdapter,
  model: {
    name: "candles15",
    ...
    upsert: {
      ...
      async handler(ctx: Context<{ entities: cpz.Candle[] }>) {
        try {
          ...
          const query = `INSERT INTO candles15 (id, exchange, asset, currency, open, high, low, close, volume, time, timestamp, type) VALUES
          ${values
            .map(_ => {
              return "(?)";
            })
            .join(",")} 
           ON CONFLICT ON CONSTRAINT candles15_time_exchange_asset_currency_key 
