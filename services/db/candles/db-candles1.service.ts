    ...
    upsert: {
      ...
      async handler(ctx) {
        try {
          ...
          const query = `INSERT INTO candles1 (id, exchange, asset, currency, open, high, low, close, volume, time, timestamp, type) VALUES
          ${values
            .map((_: cpz.DBCandle) => {
              return "(?)";
            })
            .join(",")} 
           ON CONFLICT ON CONSTRAINT candles1_time_exchange_asset_currency_key 
