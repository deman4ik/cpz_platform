...
const CandlesService: ServiceSchema = {
  name: `${cpz.Service.DB_CANDLES}480`,
  mixins: [DbService],
    ...
    upsert: {
      ...
      async handler(ctx: Context<{ entities: cpz.Candle[] }>) {
        try {
          ...
          const query = `INSERT INTO candles480 (id, exchange, asset, currency, open, high, low, close, volume, time, timestamp, type) VALUES
          ${values
            .map(_ => {
              return "(?)";
            })
            .join(",")} 
           ON CONFLICT ON CONSTRAINT candles480_time_exchange_asset_currency_key 
