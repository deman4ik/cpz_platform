    ...
    upsert: {
      ...
      async handler(ctx: Context<{ entities: cpz.Candle[] }>) {
        try {
          ...
          const query = `INSERT INTO candles5 (id, exchange, asset, currency, open, high, low, close, volume, time, timestamp, type) VALUES
          ${values
            .map(_ => {
              return "(?)";
            })
            .join(",")} 
           ON CONFLICT ON CONSTRAINT candles5_time_exchange_asset_currency_key 
