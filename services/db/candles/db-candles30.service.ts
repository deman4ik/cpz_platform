...
const CandlesService: ServiceSchema = {
  name: `${cpz.Service.DB_CANDLES}30`,
  mixins: [DbService],
  adapter,
  model: {
    name: "candles30",
    ...
    upsert: {
      ...
      async handler(ctx: Context<{ entities: cpz.Candle[] }>) {
        try {
          ...
          const query = `INSERT INTO candles30 (exchange, asset, currency, open, high, low, close, volume, time, timestamp, type) VALUES
          ${values
            .map((_) => {
              return "(?)";
            })
            .join(",")} 
           ON CONFLICT ON CONSTRAINT candles30_time_exchange_asset_currency_key 
